...
//	printf("k&l calculations\n");
	for (i = 0; i < len; ++i) {
		...
		if (c < 4) {
#if USE_OCC4 == 1
//			printf("using BWTOCC4 for k \n");
			bwt_occ4(bwt, k-1, ks);
//			printf("using BWTOCC4 for l\n");
			bwt_occ4(bwt, l, ls);
			k = bwt->L2[c] + ks[c] + 1;
			l = bwt->L2[c] + ls[c];

#else
//			printf("using BWTOCC2 for k & l\n");
			bwt_2occ(bwt, k - 1, l, c, &ok, &ol);
			//if(len<70) printf("\ninitial k&l: %llu,%llu\n",k,l);
			unsigned long long startK = k, startL = l;
			k = bwt->L2[c] + ok + 1;
			l = bwt->L2[c] + ol;
			//if(len<70) printf("%i	occ: %llu	k: %llu->%llu	l: %llu->%llu\n", c, bwt->L2[c], startK, ok + 1, startL, ol);

#endif
//			printf("i:%d,c:%d,bwt:%lu,k:%lu,l:%lu\n", i, c, bwt->L2[c], k, l);
...
void bwa_aln_core(const char *prefix, const char *fn_fa, gap_opt_t *opt)
{
	...
	///////////////////////////////////////
	fprintf(stderr, "[bwa_aln_core] Running BWA CPU mode... \n");
	if(opt->n_threads == 0)
	{
		...
		fprintf(stderr, "[bwa_aln_core] %d CPU(s) detected, running BWA with %d threads... \n",opt->n_threads,opt->n_threads);
	}
	...
	if(opt->n_threads >= 0)
	{
		...
		if(opt->max_entries < 0) opt->max_entries = 2000000;
...
#else
				if(!p->n_aln) continue;
				fprintf(stdout, "Sequence %i, no of alignments: %i\n", i, p->n_aln);
				int j;
				for(j=0; j < (p->n_aln); j++){
					bwt_aln1_t * aln =  p->aln + j;
					fprintf(stdout, "  Aligned read, n_mm: %i, n_gape: %i, n_gapo: %i, k: %llu, l: %llu, score: %i\n", aln->n_mm, aln->n_gape, aln->n_gapo, aln->k, aln->l, aln->score);
				}
#endif
			}
			fprintf(stderr, "%.2f sec\n", (float)(clock() - t) / CLOCKS_PER_SEC); t = clock();
...
//		fprintf(stderr, "         -c        input sequences are in the color space\n");
	...
	if (opt->fnr > 0.0) {
		...
		for (i = 17, k = 0; i <= 250; ++i) {
			...
			if (l != k) fprintf(stderr, "[aln] %dbp reads: max_diff = %d\n", i, l);
			k = l;
		}
	}
