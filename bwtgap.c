						  ...
						  bwt_width_t *seed_width, const gap_opt_t *opt, int *_n_aln, gap_stack_t *stack)
{
	//int aa;
	//for(aa=0; aa<32; aa++){
//		fprintf(stdout, "\nbid: %i	w: %llu", seed_width[aa].bid, seed_width[aa].w);
	//}
	...
	while (stack->n_entries) {
		...
		if (i > 0) {
			...
			if (seed_width && ii > 0) {
				if (seed_width[ii-1].bid > m_seed-1) {
					allow_diff = 0;
					//printf("\n...%i	%i	%i", len-i, seed_width[ii-1].bid, m_seed);
				}
				else {
					if (seed_width[ii-1].bid == m_seed-1 && seed_width[ii].bid == m_seed-1
		...
		if (allow_diff && allow_M) { // mismatch is allowed
			for (j = 1; j <= 4; ++j) {
				...
				l = bwt->L2[c] + cnt_l[c];
				if (k <= l) {
					gap_push(stack, i, k, l, e.n_mm + is_mm, e.n_gapo, e.n_gape, STATE_M, is_mm, opt);
#if ARRAN_DEBUG_LEVEL > 0 && ARRAN_PRINT_DFS > 0 && ARRAN_PRINT_MISMATCHES > 0
	...
	printf("\n### M %i %c", len - i, bases[c]);
			...
			l = bwt->L2[c] + cnt_l[c];
			if (k <= l) {
				gap_push(stack, i, k, l, e.n_mm, e.n_gapo, e.n_gape, STATE_M, 0, opt);
			}
		}
	}
