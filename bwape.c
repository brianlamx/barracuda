...
static int infer_isize(int n_seqs, bwa_seq_t *seqs[2], isize_info_t *ii, double ap_prior, int64_t L, int verbose)
{
	...
	if (tot < 20) {
		if (verbose) fprintf(stderr, "[infer_isize] fail to infer insert size: too few good pairs\n");
		free(isizes);
		return -1;
	}
	...
	free(isizes);
	if (verbose) fprintf(stderr, "[infer_isize] (25, 50, 75) percentile: (%d, %d, %d)\n", p25, p50, p75);
	if (isnan(ii->std) || p75 > 100000) {
		ii->low = ii->high = ii->high_bayesian = 0; ii->avg = ii->std = -1.0;
		if (verbose) fprintf(stderr, "[infer_isize] fail to infer insert size: weird pairing\n");
		return -1;
	}
	for (y = 1.0; y < 10.0; y += 0.01)
		if (.5 * erfc(y / M_SQRT2) < ap_prior / L * (y * ii->std + ii->avg)) break;
	ii->high_bayesian = (bwtint_t)(y * ii->std + ii->avg + .499);
	if (verbose) fprintf(stderr, "[infer_isize] low and high boundaries: %ld and %ld for estimating avg and std\n", (long)ii->low, (long)ii->high);
	if (verbose) fprintf(stderr, "[infer_isize] inferred external isize from %d pairs: %.3lf +/- %.3lf\n", n, ii->avg, ii->std);
	if (verbose) fprintf(stderr, "[infer_isize] skewness: %.3lf; kurtosis: %.3lf; ap_prior: %.2e\n", skewness, kurtosis, ii->ap_prior);
	if (verbose) fprintf(stderr, "[infer_isize] inferred maximum insert size: %ld (%.2lf sigma)\n", (long)ii->high_bayesian, y);
	return 0;
}
...
void posix_pe(const bntseq_t *bns, int n_seqs, bwa_seq_t *seqs[2], aln_buf_t* buf[2], /*pe_data_t *d, */const pe_opt_t *opt, const bwt_t *bwt, const isize_info_t *ii, const gap_opt_t *gopt, int tid, int n_threads)
{
	int i,j;
	int cnt_chg = 0;
	pe_data_t *d;
	...
	if(opt->verbose) fprintf(stderr, "  [posix_pe] Thread %d: Change of coordinates in %d alignments.\n", tid, cnt_chg);

}


//POSIX SE
void posix_se(const bntseq_t *bns, int n_seqs, bwa_seq_t *seqs[2], const bwt_t *bwt, aln_buf_t *buf[2], const gap_opt_t *gopt, int tid, int n_threads)
{
	int i, j;
...
static void *sampe_pe_worker(void *data)
{
	...
	posix_pe(d->bns, d->n_seqs, d->seqs, d->buf, /*d->d,*/ d->opt, d->bwt, d->ii, d->gopt, d->tid, d->n_threads);
	return 0;
}
...
{
	thread_sampe_pe_aux_t *d = (thread_sampe_pe_aux_t*)data;
	posix_se(d->bns, d->n_seqs, d->seqs, d->bwt, d->buf, d->gopt, d->tid, d->n_threads);
	return 0;
}
					   ...
					   const pe_opt_t *opt, const gap_opt_t *gopt, const isize_info_t *last_ii, int n_threads)
{
	...
	if (n_threads <= 1) { // no multi-threading at all
		posix_se(bns, n_seqs, seqs, bwt, buf, gopt, 0, 0);
	} else {
		pthread_t *tid;
		pthread_attr_t attr;
...
#ifdef HAVE_PTHREAD
	if (n_threads <= 1) { // no multi-threading at all
		posix_pe(bns, n_seqs, seqs, buf,/* d,*/ opt, bwt, ii, gopt, 0, 0);
	} else {
		pthread_t *tid;
		pthread_attr_t attr;
		thread_sampe_pe_aux_t *data;
...
void bwa_paired_sw(const bntseq_t *bns,
...
#define __set_fixed(_pref, _pmate, _beg, _cnt) do {						\
	...
	if(popt->verbose)
	{
		fprintf(stderr, "[bwa_paired_sw] Thread %d: %lld out of %lld Q%d singletons are mated.\n", tid,
				(long long)n_mapped[1], (long long)n_tot[1], SW_MIN_MAPQ);
		fprintf(stderr, "[bwa_paired_sw] Thread %d: %lld out of %lld Q%d discordant pairs are fixed.\n", tid,
...
#define BATCH_SIZE 0x80000
	...
	while ((seqs[0] = bwa_read_seq(ks[0], BATCH_SIZE, &n_seqs, opt0.mode, opt0.trim_qual)) != 0) {
		...
		bwa_cal_pac_pos_pe(bns, prefix, bwt, n_seqs, seqs, fp_sa, &ii, popt, &opt, &last_ii,n_threads);
		gettimeofday (&end, NULL);
		time_used = diff_in_seconds(&end,&start);
		total_time_used += time_used;
		...
		if(popt->verbose) fprintf(stderr, "[sampe_core] Aligning unmapped mate...\n");
		gettimeofday (&start, NULL);
		...
		if(popt->verbose) fprintf(stderr, "[sampe_core] Refining gapped alignments...\n");
		gettimeofday (&start, NULL);
		for (j = 0; j < 2; ++j)
	...
	// destroy
	if(popt->verbose) {
		fprintf(stderr, "[sampe_core] Cleaning up...\n");
	}
	bns_destroy(bns);
...
int bwa_sai2sam_pe(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "a:o:sPn:N:c:f:AVr:t:")) >= 0) {
		switch (c) {
		case 'r':
		...
		case 't': popt->thread = atof(optarg); break;
		case 'V': popt->verbose = 1; break;
		default: return 1;
		}
	}
	...
	if (optind + 5 > argc) {
		fprintf(stderr, "\n");
		fprintf(stderr, "Usage:   barracuda sampe [options] <prefix> <in1.sai> <in2.sai> <in1.fq> <in2.fq>\n\n");
		...
		fprintf(stderr, "         -V       Verbose mode, default: silent\n\n");
